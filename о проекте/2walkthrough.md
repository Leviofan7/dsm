...
graph TD
    ...
    subgraph Цикл Observe-Think-Act (макс. 12 шагов)
        BrowserStart --> Resilience[🛡️ Проверка 403 / Captcha]
        
        Resilience -->|Блок обнаружен| Retry1[Попытка 1: Сброс Cookies + Ожидание]
        Retry1 --> Resilience
        Resilience -->|Блок после Попытки 1| Retry2[Попытка 2: Смена User-Agent + Ожидание]
        Retry2 --> Resilience
        Resilience -->|Все попытки исчерпаны| StopBlocked[Остановка: Вывод скриншота блокировки]
        
        Resilience -->|Доступ разрешен| Screenshot[1. Observe: Скриншот видимой зоны]
        Screenshot --> Vision[2. Think: Vision-модель + credentials]
        Vision --> Act{3. Act: Действие?}
        
        Act -->|click x, y| ExecuteClick[Клик по координатам] --> Resilience
        Act -->|type text| ExecuteType[Ввод текста] --> Resilience
...
* **Скриншоты блокировок:** Если агент получает ошибку доступа, он прикрепляет `base64` снимок экрана, который можно развернуть прямо в чате нажатием на кнопку «Показать скриншот блокировки».
* **Цветовая индикация состояния:**
  * 🟡 **Желтый/Фиолетовый** — агент работает.
  * 🟢 **Зеленый** — задача успешно решена.
  * 🔴 **Красный** — произошла ошибка.
* **Кастомные иконки:** Каждый шаг логически визуализирован (эмуляция браузера 🌐, анализ экрана 📷, блокировки 🛡️, генерация ответа ⚡).

### 5. 🛡️ Устойчивость к блокировкам (Resilience)
Встроенный модуль обхода защит от ботов (Cloudflare, 403 Forbidden, Captcha):
* **Детектирование:** Агент анализирует заголовок и `body` страницы на наличие маркерных слов блокировки на каждом шаге цикла.
* **Мягкий ретрай:** При первом обнаружении блока агент имитирует чтение (пауза 5-10 секунд) и сбрасывает Cookies.
* **Жесткий ретрай:** При повторной блокировке агент полностью пересоздает Browser Context, подменяя `User-Agent` на новый из пула случайных десктопных юзер-агентов, и повторяет паузу.
* **Отчетность:** Если после 2 попыток блокировка не снята, цикл завершается, и в чат возвращается визуальный отчет с ошибкой и скриншотом капчи/заглушки.
